...
class CounterGroup {
public:
  CounterGroup() : _leader_fd(-1), _started(false) {
  ...
  CounterSnapshot read() const {
    ...
    auto readCounter = [](int fd) -> uint64_t {
      if (fd < 0)
        return 0;
      uint64_t value = 0;
    ...
    snap.llc_load_misses = readCounter(_fds[0]);
    snap.llc_loads = readCounter(_fds[1]);
    snap.l1d_load_misses = readCounter(_fds[2]);
    snap.l1d_loads = readCounter(_fds[3]);
    snap.dtlb_load_misses = readCounter(_fds[4]);
    snap.dtlb_loads = readCounter(_fds[5]);
    snap.instructions = readCounter(_fds[6]);
    snap.cycles = readCounter(_fds[7]);
