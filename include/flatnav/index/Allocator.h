...
struct MemoryAllocParameters {
  ...
  MemoryAllocParameters() = default;

  MemoryAllocParameters(size_t data_size_bytes, size_t M, size_t max_node_count,
                        DataType data_type)
  ...
  void serialize(Archive& archive) {
    archive(data_size_bytes, M, max_node_count, node_size_bytes, data_type);
  }

  static std::shared_ptr<MemoryAllocParameters> load(const std::string& filename) {
    std::ifstream stream(filename, std::ios::binary);
    if (!stream.is_open()) {
      throw std::runtime_error("Unable to open file for reading: " + filename);
    }

    auto params = std::make_shared<MemoryAllocParameters>();
...
class FlatMemoryAllocator {
 ...
 public:
  FlatMemoryAllocator(MemoryAllocParameters* params) : _params(params) {
  ...
  const MemoryAllocParameters& getParams() const { return *_params; }
};
