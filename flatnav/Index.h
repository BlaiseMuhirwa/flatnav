...
private:
    ...
    typedef std::priority_queue<
        dist_node_t, std::vector<dist_node_t>, CompareNodes> PriorityQueue;
    ...
    void selectNeighbors(PriorityQueue& neighbors, const int M){
        ...
        while(candidates.size() > 0){
            ...
            for (const dist_node_t& second_pair : saved_candidates) {
                float cur_dist = distance.distance(
                    nodeData(second_pair.second), nodeData(current_pair.second));
    ...
    void connectNeighbors(PriorityQueue& neighbors, node_id_t new_node_id){
        ...
        while(neighbors.size() > 0){
            ...
            if (!is_inserted){
                ...
                float max_dist = distance.distance(
                    nodeData(new_node_id), nodeData(neighbor_node_id));
                PriorityQueue candidates;
...
public:
    Index(DistanceInterface<dist_t> dist, int num_data, int max_edges):
            distance(std::move(dist)), max_num_nodes(num_data),
            cur_num_nodes(0), M(max_edges), is_visited(num_data+1) {
        
        data_size_bytes = distance.data_size();
        node_size_bytes = data_size_bytes +
                          sizeof(node_id_t) * M +
                          sizeof(label_t);
    ...
    bool add(void* data,
        ...
        if (new_node_id > 0){
            PriorityQueue neighbors = beamSearch(data,
                                                 entry_node,
                                                 ef_construction);
    ...
    std::vector<dist_label_t> search(const void* query,
        ...
        node_id_t entry_node = searchInitialization(transformed_query,
                                                    num_initializations);
        PriorityQueue neighbors = beamSearch(transformed_query,
                                             entry_node,
                                             ef_search);
    ...
    void deserialize(std::ifstream& in){
        ...
        size_t node_size_bytes_check = data_size_bytes +
                                       sizeof(node_id_t) * M +
                                       sizeof(label_t);
    ...
    void reorder_gorder(const int window_size = 5){
        ...
        std::vector<node_id_t> P = g_order<node_id_t>(
            outdegree_table, window_size);
