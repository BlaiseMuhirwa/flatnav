...
#include <vector>
  ...
  public:
    ...
    void setParameters(const float *centroids_, int m) {
      ...
      auto bytes_to_copy =
          _subq_centroids_count * _subvector_dim * sizeof(float);
     ...
     */
    void computePQCodes(const float *vectors, uint8_t *codes,
                        uint64_t n) const {
    ...
    void train(const float *vectors, uint64_t n) {
      ...
      for (uint32_t m = 0; m < _num_subquantizers; m++) {
        ...
        case TrainType::HOT_START:
          std::memcpy((void *)centroids_generator.centroids(),
                      getCentroids(m, 0),
...
#pragma omp parallel for if (n > 1)
    ...
    /**
     * @brief Computes the distance between a query vector and a database
     * vector. NOTE: The first vector is expected to the query vector and the
     * second one a database vector.
     *
  ...
  private:
    // NOTE: This is a hack to get around the fact that the PQ class needs to
    // know which distance function to use. So, this function allows us to just
    // extract the distance function pointer since that's the only thing we care
    // about. There's gotta be a cleaner way to not have to do this, but this
    // will do for now.
     ...
     *
     * @TODO: Use BLAS in regimes where _subvector_dim is large since we can
     * gain some speed.
     *
     */
...
#pragma omp parallel for
    ...
    // Represents centroids in a transposed form. This is useful while
    // performing the Asymmetric Distance Computation (ADC) where we are able to
    // use the transposed centroids to leverage SIMD instructions. Layout:
    // (_subvector_dim x _num_subquantizers x _subq_centroids_count)
    ...
    enum TrainType {
      ...
      HYPERCUBE_PCA, // Initialize centroids with nbits-D hypercube post PCA
                     ...
                     // seems to believe that this is a good initialization, so
                     // we might test it out to see if it actually works well.
    };
