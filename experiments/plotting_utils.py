...
def plot_percentile_against_recall(
    save_filepath: str, all_metrics: dict, percentile_key: str
):
    """
    ...
    # Adding labels and title
    plt.xlabel("Recall")
    plt.ylabel(f"{percentile_key} Latency (ms)")
    plt.title(
        f"Recall/{percentile_key} Latency Tradeoff - down and to the right is better"
    )
    plt.legend()
    plt.ticklabel_format(style="plain", axis="x")
    plt.savefig(save_filepath, dpi=300, bbox_inches="tight")
